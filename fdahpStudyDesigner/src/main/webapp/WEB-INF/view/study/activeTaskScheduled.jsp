...
<div id="schedule" class="tab-pane fade in  mt-xlg">
    ...
    ${(activeTaskBo.isDuplicate > 0) ?'disabled' : ''}> <label
            for="monthlyRadio4">Monthly</label>
                                                          ...
                                                          id="manuallyRadio5" class="schedule"
    ...
    ${(activeTaskBo.isDuplicate > 0)?'disabled' : ''}> <label
            for="manuallyRadio5">Custom Schedule</label>
                                                          ...
                                                          id="ongoingRadio6" class="schedule"
    ...
    <div class="oneTime all mt-lg">
        ...
        <div class="mt-sm">
			<span class="checkbox checkbox-inline"> <input type="hidden"
                                                           name="activeTaskFrequenciesBo.id"
                                                           id="oneTimeFreId"
        ...
        <div class="mt-sm">
			<span class="checkbox checkbox-inline"> <input type="checkbox"
                                                           id="isStudyLifeTime"
                                                           name="activeTaskFrequenciesBo.isStudyLifeTime"
                                                           value="true"
			...
			<span class="weeklyStartCls  col-md-3 pl-none"> <span
					...
					<span class="gray-xs-f">Start date<span class="requiredStar">
							* </span></span><br/> <input id="startWeeklyDate" type="text"
                                                         class="form-control mt-sm calendar ${(activeTaskBo.isDuplicate > 0)?'cursor-none' : ''}"
                                                         required name="activeTaskLifetimeStart"
                                                         placeholder="Choose Date"
                ...
                <div class="manually-anchor-option mb-md form-group" id="0">
                    <input type="hidden"
						...
						<span class="light-txt opacity06"> Anchor Date </span>
                                           ...
                                           name="activeTaskCustomScheduleBo[0].xDaysSign"
                                           id="xSign0">
							<option value="0"
                        ...
                        style="padding-right: 5px; padding-left: 5px">to </span> Anchor
							Date
                                           ...
                                           count='0' title="Select"
                                           name="activeTaskCustomScheduleBo[0].yDaysSign"
                                           id="ySign0">
							<option value="0"
...
$('span.remBtnDis').remove();
      ...
      $(document).ready(function () {
        ...
        $(document).on('change dp.change ', '.dailyClock', function () {
            ...
            if (!chkVal) {
              thisAttr.parents('.dailyTimeDiv').find('.dailyClock').parent().find(
                  ...
                  $("<ul><li> </li></ul>").attr("class", "list-unstyled").attr("style",
                      ...
                      "Please select a time that has not been added already."));
            } else {
              thisAttr.parents('.dailyTimeDiv').find('.dailyClock').parent().find(
                  ".help-block").empty();
            }
          ...
          var a = 0;
          $('.dailyContainer').find('.dailyTimeDiv').each(function () {
            if ($(this).find('.dailyClock').parent().find(
                '.help-block.with-errors').children().length > 0) {
              a++;
            }
          });
          multiTimeVal = !(a > 0);
...
//             endDate = formatDate(dt);
        ...
        $("#startDateWeekly").on('change', function () {
          ...
          $('#startWeeklyDate').not('.cursor-none, :disabled').datetimepicker({
            ...
            var weeklyDate = $("#startWeeklyDate").val();
            var weeks = $("#weeks").val();
            if ((weeklyDate != null && weeklyDate != '' && typeof weeklyDate != 'undefined')
                && (weeks != null && weeks != '' && typeof weeks != 'undefined')) {
              var dt = new Date(weeklyDate);
              var weekcount = Number(weeks) * 7;
              console.log(weekcount)
              dt.setDate(dt.getDate() + Number(weekcount));
...
//             endDate = formatDate(dt);
        ...
        $(document).on('click change dp.change', '.cusStrDate, .cusTime', function (e) {
          ...
          if ($(this).is('.cusStrDate') && !$(this).parents('.manually-option').find(
              '.cusTime').prop('disabled')) {
...
// 	updateLogoutCsrf();
      ...
      function isNumber(evt, thisAttr) {
        ...
        if ((charCode < 48 && charCode > 57) || (charCode >= 65 && charCode <= 90) || (charCode
            >= 97 && charCode <= 122)) {
          return false;
        }
      ...
      function saveActiveTask(item, actType, callback) {
        ...
        if (frequency_text != null && frequency_text != '' && typeof frequency_text
            != 'undefined') {
        ...
        if (frequency_text == 'One time') {
          ...
          if (frequency_date != null && frequency_date != '' && typeof frequency_date
              != 'undefined') {
          ...
          if (isLaunch_study != null && isLaunch_study != '' && typeof isLaunch_study
              != 'undefined') {
          ...
          if (scheduletype == 'AnchorDate') {
            $('.manually-anchor-option').each(function () {
              var activeTaskCustomFrequencey = {};
...
// 		}
          ...
          $('.time-opts').each(function () {
            ...
            if (dailyXSign != null && dailyXSign != '' && typeof dailyXSign != 'undefined' && count
              ...
              if (dailyXSign == '0')
                xval = false;
              activeTaskFrequencey.xDaysSign = xval;
              if (dailyXSignVal != null && dailyXSignVal != '' && typeof dailyXSignVal
                  != 'undefined') {
          ...
          if (study_lifetime_start != null && study_lifetime_start != ''
              && typeof study_lifetime_start != 'undefined') {
		...
		$(".weeklyRegular").removeAttr('required'); */
          ...
          if (weeklyXSignVal != null && weeklyXSignVal != '' && typeof weeklyXSignVal
              != 'undefined') {
          ...
          activeTask.activeTaskFrequenciesBo = activeTaskFrequencey;
          if ($('#weeklyFormId').find('.numChk').val() && $('#weeklyFormId').find('.numChk').val()
              == 0 || !validateTime(
                  $(document).find("#startWeeklyDate").not('.cursor-none, :disabled'),
          ...
          if (frequencetime_anchor != null && frequencetime_anchor != ''
              && typeof frequencetime_anchor != 'undefined') {
...
//					  	showErrMsg("Something went Wrong");
      ...
      function checkDateRange() {
        $(document).on('dp.change change', '.cusStrDate, .cusEndDate, .cusTime', function (e) {
          var chkVal = true;
          if ($(this).parents('.manually-option').find('.cusStrDate').val() && $(this).parents(
              '.manually-option').find('.cusEndDate').val() && $(this).parents(
              '.manually-option').find('.cusTime').val()) {
            var thisAttr = this;
            $(this).parents('.manuallyContainer').find('.manually-option').each(function () {
              if ((!$(thisAttr).parents('.manually-option').is($(this))) && $(this).find(
                ...
                var thisFromDate = moment(
                    ...
                    "MM/DD/YYYY").toDate();
                var thisCusTime = moment(
                    $(thisAttr).parents('.manually-option').find('.cusTime').val(),
                    "HH:mm").toDate()
          ...
          if (!chkVal) {
            $(thisAttr).parents('.manually-option').find('.cusTime').parent().addClass(
                'has-error has-danger').find(".help-block").removeClass(
                'with-errors').empty().append(
                $("<ul><li> </li></ul>").attr("class", "list-unstyled").attr("style",
                    "font-size: 10px;").text(
      ...
      function setFrequencyVal(flag) {
        ...
        if (frequencyType) {
          ...
          if (frequencyType == 'One time') {
            ...
            $('.addLineChartBlock_Number_of_Failures_spatial').find('.requireClass').prop(
                'required', false);
            ...
            $('.addLineChartBlock_Score_spatial').find('.requireClass').prop('required',
                'required');
            ...
            $("#chartId,#chartId1,#chartId2").attr('required', 'required').prop('required',
                'required');
        ...
        dateRef.each(function () {
          ...
          if (dt) {
            ...
            if (dt < serverDate()) {
              ...
              $(this).parent().removeClass('has-error has-danger').find(
      ...
      var chkEndDateWithDate = function (couterRef, endDateRef) {
        ...
        if (dt && (couterRef.val() !== 0)) {
          if (moment(dt, "MM/DD/YYYY").toDate() < serverDateTime()) {
            couterRef.parent().addClass('has-error has-danger').find(
                '.help-block.with-errors').empty().append(
                $("<ul><li> </li></ul>").attr("class", "list-unstyled").text(
                    "Please ensure the End Date/Time is greater than current date/time."));
            valid = false;
          } else {
            couterRef.parent().removeClass('has-error has-danger').find(
                '.help-block.with-errors').empty();
          }
        }
        return valid;
      ...
      //function addDateAnchor() {
        ...
        var newDateCon = "<div class='manually-anchor-option mb-md form-group' id='"
            ...
            + "' type='text' class='form-control wid70 disRadBtn1 disBtn1 ml-1 remReqOnSave xdays daysMask resetAncDate xancorText'"
            ...
            + "' type='text' class='form-control wid70 disRadBtn1 ml-1 disBtn1 remReqOnSave ydays daysMask resetAncDate yancorText' count='"
            ...
            + "<input id='manualTime" + customAnchorCount + "' type='text' count='"
            + customAnchorCount + "' class='form-control clock' name='activeTaskCustomScheduleBo["
